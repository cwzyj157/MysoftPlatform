...
namespace SmokingTest.DAL
{
	...
	public class TestAncillaryUpdateCode
	{
		...
		public void Update_TrackChange()
		{
			...
			CPQuery cp = null;
			t1.HookExecute(p => { cp = p; return false; }
				);
			t1.Update();
			...
			//验证修改成功
			Assert.AreEqual(cp.ExecuteNonQuery(), 1);
		}
		/// <summary>
		/// TrackChange 原始值修改
		/// 验证参数是否按需加载
		...
		/// <summary>
		/// 验证原始值类型的并发修改，取原始值的方式为NEW的方式,new 的对象为空
		/// </summary>
		...
		public void ConcurrencyUpdate_OriginalValue1()
		{
			...
			TestPackage.AssertException<ArgumentNullException>(() => {
				 t1.Update(null, ConcurrencyMode.OriginalValue);
			});
