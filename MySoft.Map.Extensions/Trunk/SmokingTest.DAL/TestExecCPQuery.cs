...
namespace SmokingTest.DAL
{
	...
	public class TestExecCPQuery : IDisposable
	{
		...
		public void _Smoking_0_InitTestTable()
		{
			...
			// 重新创建测试用例所需要的存储过程
			@"CREATE PROC usp_TestParameterNull
				(
					@intVal int,
					@strVal varchar(128),
				...
				AS
				SELECT @intVal,@strVal,@guidVal,@dtmVAL"
				.AsCPQuery().ExecuteNonQuery();
		}


		private void DropTestProc()
		{
			var query = "SELECT count(1) FROM dbo.sysobjects WHERE id=OBJECT_ID(N'[dbo].[usp_TestParameterNull]') AND OBJECTPROPERTY(id,   N'IsProcedure') = 1".AsCPQuery();
			long objectId = query.ExecuteScalar<long>();

			if( objectId > 0 )
				"drop proc [usp_TestParameterNull]".AsCPQuery().ExecuteNonQuery();
		...
		[TestMethod(Order = 5)]
		[NUnit.Framework.Test]
		public void _Smoking_4_TextExecNull()
		{
			
			var parameter = new {
				intVal = DBNull.Value,
				strVal = DBNull.Value,
				guidVal = DBNull.Value,
				dtmVAL = DBNull.Value
			};

			CPQuery query =CPQuery.From("exec usp_TestParameterNull @intVal, @strVal, @guidVal, @dtmVAL", parameter);

			DataTable dt = query.FillDataTable();


			StoreProcedure sp = new StoreProcedure("usp_TestParameterNull", parameter);
			dt = sp.FillDataTable();
		}
