...
namespace TaskManagerLib.DAL
{
	...
	public class MyMongoDb : IDisposable
	{
		...
		private static readonly string s_defaultDatabaseName = "TaskManger";

		static MyMongoDb()
		{
			ConnectionStringSettings setting = ConfigurationManager.ConnectionStrings["MongoDb"];
			if( setting != null && string.IsNullOrEmpty(setting.ConnectionString) == false )
				s_connectionString = setting.ConnectionString;
			else
				s_connectionString = "Server=127.0.0.1";
		}
		...
		/// </summary>
		public MyMongoDb()
			: this(s_connectionString, s_defaultDatabaseName)
		{
		}
		...
		public IMongoDatabase CurrentDb
		{
			...
			{
				if( _db == null )
					throw new InvalidOperationException("当前连接没有指定任何数据库。请在构造函数中指定数据库名或者调用UseDb()方法切换数据库。");
